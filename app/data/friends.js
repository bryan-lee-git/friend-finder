    ...
    checkAllFriends: async function (newFriend) {
        ...
        await this.friends.forEach(friend => {
            newFriend.checkForBestFriend(friend).then(diff => {
                if (friend.name != newFriend.name && diff < 14) match.push(friend);
            });
        });
        return match;
    }
...
// --- RANDOM FRIEND CREATION USING FRIEND CONSTRUCTOR ---
...
var Kat = new Friend("Kat Purifoy", "https://randomuser.me/api/portraits/women/92.jpg");
Kat.getRandomStats();
